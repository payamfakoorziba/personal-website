...
<Callout>
💡 Make sure the conditional statement is **_inside_** of `AnimatePresence`. So this doesn’t work:
...
We can achieve this by using the `transition` prop. We can pass an object to `transition` that tells Motion the **_type_** of animation we want with things like `duration`, `delay` and `ease` . Here’s a staggered balls animation:
...
<Callout>
💡 If you’re experiencing a weird “jump” when animating to `height: "auto"` it
...
```tsx
// ❌ This causes a jump because the animating element has padding
<motion.div
  initial={{ height: 0 }}
  animate={{ height: "auto" }}
  exit={{ height: 0 }}
...
// ✅ Moving the padding to a child div fixes it
<motion.div
  initial={{ height: 0 }}
  animate={{ height: "auto" }}
  exit={{ height: 0 }}
...
<Callout>
💡 Be careful not to pass the ref to the motion element directly. In that case we’re measuring the height of the div and passing it to itself, so the height doesn’t adjust properly. We want to measure the height of the child and animate the height of the parent based on that.
...
Now we need a way to pass the direction to our variants, so that our variants become [dynamic](https://motion.dev/docs/react-animation#dynamic-variants). We can do this via the `custom` prop. We’ll pass the direction to the custom prop, and redefine our variants to use the direction as a parameter.
